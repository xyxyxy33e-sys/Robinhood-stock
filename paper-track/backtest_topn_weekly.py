"""Weekly-rebalanced backtest of the SPMO core mirror using the reconstructed
holdings history (data/spmo_holdings_history.csv) and weekly prices
(data/spmo_weekly_prices.csv). Tests top-N holdings under two weighting
schemes, rebalanced back to target weights every week:

  - proportional: weight_i = reported SPMO pct_val_i / sum(pct_val over top N)
  - equal: weight_i = 1/N

Every combo of N in {15, 25} x {proportional, equal} is reported.
"""
import csv
import math
