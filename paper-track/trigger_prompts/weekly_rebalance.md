...
uninvested cash counts toward the cash leg. Then call `state.py`'s own gate:
  ...
  - **regime changed → always rebalance**, whatever the drift. Regime =
    ...
    trim vote count (`extension_votes(...)`) plus the state-D gate flag
    (`d_gate_active(...)`, 2026-09-19), so the fast re-entry overlay
    switching, the trim changing by a step, or the D gate switching
    (100% QLD <-> 100% BOXX) counts.
...
yesterday's confirmed state's weights — from **`live_target_weights`** (with
yesterday's breadth pct and gaps), i.e. the weights actually held, WITH every
overlay including the D gate and the volatility overlay (changed
...
the breadth pct and the D-gate flags/status (2026-09-19), `micro_agrees`, both realized-vol legs with the binding one
