"""Owner (2026-09-23): "25/75". Follow-up 20. The live rule (v2: TQQQ out at the first
held vote, core 1/6 per vote, one vote off per new 15-session high, whipsaw carry 3) at A bases
50/50, 40/60, 30/70 and 25/75. Research only."""
...
F._lf = open('paper-track/research_notes/fall_protection_r22_run.log', 'w'); log = F.log
def sched(s0, t0, ks, kt):
    return [(s0 * max(0, 1 - ks * v), t0 * max(0, 1 - kt * v), 1 - s0 * max(0, 1 - ks * v) - t0 * max(0, 1 - kt * v)) for v in range(4)]
F.CARRY_G = 3
BASES = [('50/50 (live)', (.5, .5)), ('40/60', (.4, .6)), ('30/70 (next spell)', (.3, .7)), ('25/75', (.25, .75))]
...
for h in F.H:
    ...
    for lab, (c0, t0) in BASES:
        ...
        R[(h, lab)] = r
log('=' * 130); log('fall_protection_r22: live rule (v2 + carry) at A bases 50/50, 40/60, 30/70, 25/75'); log('=' * 130)
for h in F.H:
    log(f"\n  {h.upper()}")
    ...
    for lab, _ in BASES:
        ...
        log(s)
log('')
for ref in ('30/70 (next spell)', '40/60', '50/50 (live)'):
    ps = [boot(F.xs(h, S[(h, '25/75')]), F.xs(h, S[(h, ref)]), 60, seed=(sum(map(ord, '2575' + ref)) + len(h)) & 0xffff)[5] for h in F.H]
    log(f"  bootstrap 25/75 vs {ref:<19} P(Sharpe not better) real {ps[0]:.2f}  proxy {ps[1]:.2f}")
F._lf.close()
